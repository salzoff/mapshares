<template>
    <div>
        ...
        <div v-for="(hint, index) in hints" row class="mt-2" :key="index">
            ...
            <v-layout row>
                <v-flex xs2 md2 class="body-2">Visible from:</v-flex>
                <v-flex xs10 md6>{{ formatDateTime(hint.visibleFrom) }}</v-flex>
            </v-layout>
            <template v-if="hint.type === boxHintTypes.LOCATION">
                <v-layout row class="mt-1">
                    <v-flex xs2 md2 class="body-2">Coordinates:</v-flex>
                    <v-flex xs10 md6>
                        {{ `Latitude: ${hint.value.position.lat}, Longitude: ${hint.value.position.lng}` }}
                    </v-flex>
                </v-layout>
                <v-layout row class="mt-1">
                    <v-flex xs2 md2 class="body-2">Range:</v-flex>
                    <v-flex xs10 md6>
            ...
            <template v-else-if="hint.type === boxHintTypes.TEXT">
                <v-layout row class="mt-1">
                    <v-flex xs2 md2 class="body-2">Text:</v-flex>
                    <v-flex xs10 md6>{{ hint.value.text }}</v-flex>
                </v-layout>
            </template>
            <template v-else-if="hint.type === boxHintTypes.IMAGE">
                <v-layout row class="mt-1">
                    <v-flex xs2 md2 class="body-2   x">Image:</v-flex>
                    <v-flex xs10 md6>
